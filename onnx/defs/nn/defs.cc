...
static const char* TCS_ver1_doc = R"DOC(
      An EdgeAI test Function: Perform random calculation 
      on the input tensor X using formula: <br/> ``` 2*((X-EX)/sqrt(E(X))) ```
)DOC";

static float default_e = 2.0;
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr(
            "e",
            "A scaling number. The default is to 2.0 ",
            AttributeProto::FLOAT,
            default_e)
        .TypeConstraint(
            "T",
        ...
        .FunctionBody(FunctionBodyHelper::BuildNodes(
            {// nodes: {outputs, op, inputs, attributes}
             ...
             {{"EX"},"Mul",{"X"},{MakeRefAttribute("e",AttributeProto::FLOAT)}},
             {{"SQRTEX"},"Sqrt",{"EX"}},
             {{"X-EX"},"Sub",{"X","EX"}},
             {{"OP"},"Div",{"X-EX","SQRTEX"}},
             {{"OUTPUT"},"Mul",{"scale","OP"}}})));
